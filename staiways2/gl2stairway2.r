gl2stairwayplot2 <- 
  function(x, 
    ...
    if(is.null(nrand)){
      ...
      if(verbose >= 3){cat("  No. of break points not specified, set to:",nrand,"\n")}
    }
    ...
    write.table(paste("pct_training:", pct_training, "# proportion of sites for training"),
                ...
                row.names=FALSE,col.names=FALSE,
                quote=FALSE, sep=" ",append=TRUE)
    write.table(paste("nrand:", nrand, "# number of random break points for each try (separated by white space)"),
                file=outfilespec,
                row.names=FALSE,col.names=FALSE,
                quote=FALSE, sep=" ",append=TRUE)
    ...
    if (run==TRUE)
    {
      ...
      if (parallel>1)
      {
        ...
        temp <- future_map(1:length(runs), runstair)
      ...
      if (er==1)
      {
        ...
        nr <- length(ff)
        ff[nr] <-  gsub("-Xmx4g", "-Xmx1g", ff[nr])
        ff[nr-1] <-  gsub("-Xmx4g", "-Xmx1g", ff[nr-1])
